...
function LoginFormContent() {
  ...
  const handleGoogleLogin = () => {
    loginWithGoogle().then((res) => {
      if (res.success) {
        toast.success(`Successfully logged in as ${res.user.name}!`);
        router.push(redirect);
      } else {
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 transition duration-150"
          />
        </div>
        ...
        <button
          ...
          className="w-full py-3.5 px-4 font-bold text-white bg-primary-600 hover:bg-primary-700 disabled:bg-primary-400 rounded-xl shadow-lg shadow-primary-500/20 active:scale-[0.98] transition flex items-center justify-center gap-2 cursor-pointer"
        >
